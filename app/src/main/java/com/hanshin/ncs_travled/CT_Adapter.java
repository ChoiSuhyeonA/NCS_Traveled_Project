...
class CT_Adapter extends BaseAdapter{
    ...
    private ArrayList<CT_Create_Item> CT_Create_Item = new ArrayList<CT_Create_Item>() ;
    ...
    public Object getItem(int position) {
        return CT_Create_Item.get(position) ;
    }

    // 아이템 데이터 추가를 위한 함수. 개발자가 원하는대로 작성 가능.
    public void addItem(Uri cover, String title, String date) {
        CT_Create_Item item = new CT_Create_Item();

        item.setCover(cover);
        item.setTitle(title);
        item.setDate(date);

        CT_Create_Item.add(item);
    }
}
