...
public class BT_GridViewAdapter extends BaseAdapter {
    ...
    Context context;
    ArrayList<Uri> seeArrayList; // 그리드뷰에서 이미지+ 비디오 이미지를 보여주는 리스트 (가상리스트)
    ...
    public View getView(final int position, View convertView, final ViewGroup parent) { final int pos = position;
        ...
        convertView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Toast.makeText(context.getApplicationContext(),"리스트",Toast.LENGTH_SHORT).show();
            }
        });
    ...
    public void delete(){
        ...
        if(i>0){
            //실질적 데이타를 삭제
            if(String.valueOf(seeArrayList.get(i-1)).contains("video")){
                int i1 = videoArrayList.size();
                if(i1>0){
                    videoArrayList.remove(i1-1);
                }

            }else if(String.valueOf(seeArrayList.get(i-1)).contains("image")){
                int i1= imageArrayList.size();
                if(i1>0){
